...
import MonthlyReworkRejectedChart from "./MonthlyReworkRejectedChart";
...
const [activeTab, setActiveTab] = useState("ALL");
  ...
  const getYearlyColors = (data: any[]) => {
    if (data.length < 2) return ["#135d7bff", "#135d7bff"];
    const [prevYear, currYear] = data;
    return currYear.value > prevYear.value ? ["#801313ff", "#00C49F"] : ["#00C49F", "#801313ff"];
  };
    ...
    async (path: string) => {
      try {
        ...
        if (json.data?.orderCountList) {
          ...
          const salesOrder = json.data.orderCountList.find((i: any) => i.label === "Sales Orders")?.value || 0;
          const completedSales =
            json.data.orderCountList.find((i: any) => i.label === "Completed Sales Orders")?.value || 0;
          ...
          const purchaseOrder = json.data.orderCountList.find((i: any) => i.label === "Purchase Orders")?.value || 0;
          const completedPurchase =
            json.data.orderCountList.find((i: any) => i.label === "Completed Purchase Orders")?.value || 0;
          const purchaseCompletePercent = purchaseOrder > 0 ? Math.round((completedPurchase / purchaseOrder) * 100) : 0;
        ...
        if (json.data?.quoteCountList) {
          ...
          const created = json.data.quoteCountList.find((i: any) => i.label === "Created Quotes")?.value || 0;
          const ordered = json.data.quoteCountList.find((i: any) => i.label === "Ordered Quotes")?.value || 0;
        ...
        if (json.data?.requestCountList) {
          ...
          const totalRequests = json.data.requestCountList.find((i: any) => i.label === "Total Requests")?.value || 0;
          const onProcessRequests =
            json.data.requestCountList.find((i: any) => i.label === "On Process Requests")?.value || 0;
  ...
  const handlePieSliceClick = async (event: any) => {
    ...
    const serviceName = label === "Rework" ? "ListReworkWorkEfforts" : "ListRejectedWorkEfforts";

    try {
      const result = await pipe(apiGet(`/services/${serviceName}`))();
      if (E.isRight(result)) {
        const json = await result.right.json();
        const ids = json?.data?.workEffortIds ?? json?.workEffortIds ?? [];
  ...
  const handleProductionSliceClick = async (event: any) => {
    ...
    const label = chart.data.labels[element.index];
    const serviceName =
      label === "Created Productions" ? "ListCreatedProductionRuns" : "ListClosedProductionRuns";
...
<div className="stats-row">
  ...
  {/* Extra Stats Cards */}
  {(activeTab === "ALL" || activeTab === "Orders") && <div className="extra-stats-card"><OrderAmountStats /></div>}
  {(activeTab === "ALL" || activeTab === "Quotes") && <div className="extra-stats-card"><QuoteItemAmounts /></div>}
</div>
    ...
    <div className="chart-card">
      ...
      <div className="chart-container">
        {orderData.length > 0 ? (
          <Pie
            ref={orderChartRef}
            data={createPieChartData(orderData)}
            options={{
              maintainAspectRatio: false,
          ...
          <div className="Preloader">Loading orders...</div>
        )}
      </div>
    </div>
  )}
    ...
    <div className="chart-card">
      ...
      <div className="chart-container">
        {quoteData.length > 0 ? (
          <Pie
            ref={quoteChartRef}
            data={createPieChartData(quoteData)}
            options={{
              maintainAspectRatio: false,
              plugins: { legend: { position: "top" } },
              animation: { duration: 1000 },
          ...
          <div className="Preloader">Loading quotes...</div>
        )}
      </div>
    </div>
  )}
    ...
    <div className="chart-card">
      ...
      <div className="chart-container">
        {requestData.length > 0 ? (
          <Pie
            ref={requestChartRef}
            data={createPieChartData(requestData)}
            options={{
              maintainAspectRatio: false,
              plugins: { legend: { position: "top" } },
              cutout: "50%",
          ...
          <div className="Preloader">Loading requests...</div>
        )}
      </div>
    </div>
  )}
        ...
        <div className="fixed-chart">
          <Pie
            ...
            options={{
              ...
              animation: { duration: 1000 },
              cutout: "60%",
              plugins: { legend: { position: "top" }, title: { display: true, text: "Production Rework & Rejected" } },
  ...
  <div className="charts-row">
    {[
      ...
      <div className="chart-card" key={idx}>
        ...
        <div className="chart-container">
          <Bar
            ref={chart.title === "Monthly Sales Orders" ? monthlySalesRef : monthlyPurchaseRef}
    ...
    {loadingMonthlyExtra ? (
      ...
      <div className="charts-row">
        <div className="chart-card" style={{ width: "95%", maxWidth: "1100px" }}>
          <h2>Monthly Production Stats</h2>
          <div className="chart-container" style={{ width: "100%", height: "400px" }}>
            <Bar
              ref={monthlyProductionRef}
              data={createBarChartData(monthlyProductionStats, "Productions", "#6b7622ff")}
              options={{ ...createBarChartOptions(monthlyProductionStats, "Monthly Production Stats"), maintainAspectRatio: false }}
            />
          </div>
        </div>
      </div>
...
{(activeTab === "ALL" || activeTab === "Work Orders") && (
  <>
    {loadingMonthlyExtra ? (
      ...
      <div className="charts-row">
        <div className="chart-card" style={{ width: "95%", maxWidth: "1100px" }}>
          <h2>Monthly Work Order Stats</h2>
          <div className="chart-container" style={{ width: "100%", height: "400px" }}>
            <Bar
              ref={monthlyWorkOrderRef}
              data={createBarChartData(monthlyWorkOrderStats, "Work Orders", "#89614bff")}
              options={{ ...createBarChartOptions(monthlyWorkOrderStats, "Monthly Work Order Stats"), maintainAspectRatio: false }}
            />
          </div>
        </div>
      </div>
                    ...
                    datasets: [
                      {
                        ...
                        backgroundColor: getYearlyColors(yearlyOrders),
                      },
                    ],
                  }}
                  options={{
